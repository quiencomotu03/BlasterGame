...
void ABlasterCharacter::AimOffset(float DeltaTime)
{
	...
	if (Speed == 0.f && bIsInAir == false) // standing still not jumping
	{
		FRotator CurruntAimRotation = FRotator(0.f, GetBaseAimRotation().Yaw, 0.f);
		FRotator DeltaAimRotation = UKismetMathLibrary::NormalizedDeltaRotator(CurruntAimRotation, StartingAimRotation);
		AO_Yaw = DeltaAimRotation.Yaw;
		bUseControllerRotationYaw = false;
	}

	if (Speed > 0.f || bIsInAir == true) // running or jumping
	{
